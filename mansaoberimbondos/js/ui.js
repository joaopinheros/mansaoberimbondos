...
/**
 * Data ISO (só a parte YYYY-MM-DD serve pra comparar) de N dias
 * atrás, à meia-noite local. Usada nos filtros de período do
 * histórico.
 */
export function isoDiasAtras(dias) {
  const d = new Date();
  d.setHours(0, 0, 0, 0);
...
/**
 * HTML de avatares sobrepostos para uma faxina feita por vários
 * moradores. Recebe uma lista de nomes; mostra no máximo 3 e um
 * "+N" se houver mais.
 */
export function avatarStack(nomes, classe = "historico-avatar") {
  const lista = (nomes ?? []).filter(Boolean);
  if (lista.length === 0) return "";

  const visiveis = lista
    .slice(0, 3)
    .map((n) => `<span class="${classe}" title="${escapar(n)}">${iniciais(n)}</span>`)
    .join("");
  const resto = lista.length > 3 ? `<span class="${classe} avatar-stack-mais">+${lista.length - 3}</span>` : "";

  return `<div class="avatar-stack">${visiveis}${resto}</div>`;
}

/**
 * HTML dos chips de cômodos/tarefas marcados numa faxina.
 */
export function chipsTarefas(tarefas) {
  const lista = (tarefas ?? []).filter(Boolean);
  if (lista.length === 0) return "";
  return `<div class="chips-tarefas">${lista
    .map((t) => `<span class="chip-tarefa">${escapar(t)}</span>`)
    .join("")}</div>`;
}

/**
 * "com Fulano e Ciclano" a partir de uma lista de nomes de
 * participantes, já excluindo `principal`. Devolve "" se não sobrar
 * ninguém.
 */
export function textoParticipantes(nomes, principal) {
  const outros = (nomes ?? []).filter((n) => n && n !== principal);
  if (outros.length === 0) return "";
  if (outros.length === 1) return `com ${outros[0]}`;
  return `com ${outros.slice(0, -1).join(", ")} e ${outros[outros.length - 1]}`;
}
