...
import { registrarFaxina, buscarHistorico } from "./faxinas.js";
 ...
 */
export async function confirmarAgendamentoFeito(agendamento, observacao) {
...
export async function buscarHistoricoUnificado(limite = 10) {
  const [feitas, { data: naoFeitas, error }] = await Promise.all([
    buscarHistorico(limite),
    supabaseClient
      .from("agendamentos")
      .select("id, data, motivo, usuarios(nome)")
